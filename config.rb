  ...
  locations.each do |location|
    location["employees"].each do |employee|
      if employee["goal"] > 0
        ...
        # Get user's issues
        r = Jiralicious.search("owner = \"#{username}\" AND (resolutiondate >= \"#{quarter_start}\" AND resolutiondate <= \"#{quarter_end}\") AND (status = 'Client Review' OR status = 'Resolved' OR status = 'Closed') AND (issuetype != \"Rework (Post-QA Bug)\")", :max_results=>'2500',:fields=>'customfield_10004,project,issuetype,issuekey,resolutiondate')
